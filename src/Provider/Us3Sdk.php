...
class Us3Sdk
{
    ...
    public function put($key_name, $contents, $headers = [])
    {
        ...
        if ($resp->getStatusCode() == 200) {
            return [$key_name, $resp->getStatusCode()];
        }

        return [$resp->getBody()->getContents(), $resp->getStatusCode()];
    }

    public function putFile($key_name, $filePath, $headers = [])
    {
        $resp = $this->httpClient->request('PUT', $key_name, [
            'headers' => $headers,
    ...
    public function get($key_name)
    {
        ...
        if ($resp->getStatusCode() != 200) {
            throw new Us3SdkException("get $key_name error :" . $resp->getStatusCode());
        }
    ...
    public function size($key_name)
    {
        $resp = $this->httpClient->head($key_name);
        if ($resp->getStatusCode() != 200) {
            throw new Us3SdkException("size $key_name error :" . $resp->getStatusCode());
        }
    ...
    public function delete($key_name)
    {
        ...
        if (!in_array($resp->getStatusCode(), [200, 204])) {
            throw new Us3SdkException("delete $key_name error :" . $resp->getStatusCode());
        }
